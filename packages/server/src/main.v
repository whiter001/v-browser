...
用法:
  ...
  v-browser network requests    列出网络请求 (--url <filter>)
  ...
  v-browser network hook start           启动 XHR Hook (--capture-body --capture-response --filter <pattern>)
  v-browser network hook stop            停止 Hook
  v-browser network hook status          查看 Hook 状态
  v-browser network hook records         查看记录 (--filter <pattern>)
